...
export default function Navbar() {
  ...
  return (
    <>
            ...
            {isLoggedIn ? (
              <>
                {isAdmin && (
                  <Link
                    to="/admin"
                    className="font-ui font-semibold uppercase text-xs tracking-[0.2em] text-terracotta hover:text-charcoal transition-colors"
                  >
                    Admin
                  </Link>
                )}
      ...
      {menuOpen && (
        <div
          className="fixed inset-0 z-[90] bg-charcoal/40 md:hidden"
          onClick={() => setMenuOpen(false)}
        />
      )}

      {/* Mobile slide-in menu — rendered outside nav for proper layering */}
      <div
        className={`fixed top-0 right-0 h-full w-72 z-[100] shadow-xl transform transition-transform duration-300 md:hidden ${
          menuOpen ? 'translate-x-0' : 'translate-x-full'
        }`}
        style={{ backgroundColor: '#faf8f4' }}
      >
        <div className="flex items-center justify-end p-4 border-b border-stone">
          <button
            type="button"
            onClick={() => setMenuOpen(false)}
            ...
            <>
              {isAdmin && (
                <Link
                  to="/admin"
                  onClick={() => setMenuOpen(false)}
                  className="font-ui font-semibold uppercase text-sm tracking-[0.2em] text-terracotta hover:text-charcoal transition-colors"
                >
                  Admin
                </Link>
              )}
